<template lang="pug">
section.section.contact-us(id="contacts")
  ._layer
...
export default {
  ...
  methods: {
    settitle () {
      ...
      this.title = this.pageLocale === 'en' ? 'Feedback' : 'Обратная связь'
      this.titleContacts = this.pageLocale === 'en' ? 'Contacts' : 'Контакты'
    },
    async getLocation () {
      // eslint-disable-next-line quote-props
      await this.$strapi.find('location', { locale: this.pageLocale })
        .then((result) => {
          this.location = result.data.attributes.Location
        })
        .catch((error) => {
    ...
    async getEmail () {
      // eslint-disable-next-line quote-props
      await this.$strapi.find('email', { locale: this.pageLocale })
        .then((result) => {
    ...
    async getPhone () {
      // eslint-disable-next-line quote-props
      await this.$strapi.find('phone', { locale: this.pageLocale })
        .then((result) => {
          this.phone = result.data.attributes.Phone
        })
        .catch((error) => {
          // eslint-disable-next-line no-console
          console.log(error)
...
.contact-us {
  ...
  ._layer {
    ...
    width: 131.5rem;
    height: 40rem;
    background: url("~@/assets/img/bg-form.svg");
